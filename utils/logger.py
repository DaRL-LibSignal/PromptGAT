...
def modify_config_file(path, config, task_id=None):
    """
    ...
    if config['command']['world'] == 'cityflow':
        ...
        # modify config step2
        file_name = os.path.join(get_output_file_path(config, task_id=task_id),  logger_param['replay_dir'])
        if config['world']['dir'] in file_name:
            file_name = file_name.strip(f"{config['world']['dir']} + '\n'")
        path_config['roadnetLogFile'] = file_name + f"/{datetime.datetime.now().strftime('%Y_%m_%d-%H_%M_%S')}.json"
        path_config['replayLogFile'] = file_name + f"/{datetime.datetime.now().strftime('%Y_%m_%d-%H_%M_%S')}.txt"
...
def get_output_file_path(config, task_id=None):
    """"
    set output path
    """
    ...
    if not task_id:
        ...
        task_id = task_id + '_' + timestamp

    path = os.path.join(config['world']['dir'] , 'output_data', param['task'], 
        f"{param['world']}_{param['agent']}", param['network'], param['prefix'] + '_' + task_id)
    return path
...
def setup_logging(level):
    ...
    if not root.hasHandlers():
        ...
        handler_file = logging.FileHandler(os.path.join(
            logger_dir,
            f"{datetime.datetime.now().strftime('%Y_%m_%d-%H_%M_%S')}_BRF.log"), mode='w'
        )
